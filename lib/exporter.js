...
class Exporter extends EventEmitter {
    ...
    async retrievePages(nodes, pages) {
        ...
        if (!nodes) {
            ...
            let maxtime = 0;
            let totaltime = 0;
            const totalcnt = pages.length;
            const promises = [];
            const batch = this.options.maxParallel || DEF_MAX_PARRALLEL;
                ...
                if (cnt < batch) {
                    promises.push(new Promise(async (resolve, reject) => {
                        ...
                        if (node.data) {
                            ...
                            console.info(node.title + " downloaded ( " + time + " ms )");
                        }
                        resolve();
                    }));
                    cnt++;
            ...
            return Promise.all(promises).then(() => {
                console.info("download time : max " + maxtime + " ms, avg " + (totaltime / totalcnt) + " ms");
            });
        }
    }
